...
platform_hinge_offset =  30;
tower_radius          = 300;

lateral_offset        =  27;     // lateral offset of the moving roads *2 = rod spacing
play                  =   0.5;   // Play required for tight adjustment

magnet_tilt           =  30;     // orientation of the magnetic joints
magnet_diam           =  19;     // Diameter of the magnet
magnet_height         =   7.3;     // Height of the magnet
...
module ventola2() {
	rotate([90, 0, 0]) translate([ 0, 0, 56])rotate([-50, 0, 0])fan(40, 8, 34);
}

module ventola3() {
	rotate([90, 0, 120]) translate([ 0, 0, 56])rotate([-50, 0, 0])fan(40, 8, 34);
}

module scasso_ventola2() {
		rotate([90, 0, 0]) translate([ 0, 0, 56])rotate([-50, 0, 0])cube([40.2, 40.2, 8],center=true);
	rotate([90, 0, 0]) translate([ 0, 0, 56])rotate([-50, 0, 0])translate([ -20.1, -20.1, -4])cube([40.2, 40.2, 20]);
		//rotate([90, 0, 0]) translate([ 0, 0, 50])rotate([-50, 0, 0])translate([ -15, -15, -30])cube([30, 30, 20]);

		rotate([90, 0, 0]) translate([ 0, 0, 56])rotate([-50, 0, 0])translate([ 17, 17, -10])cylinder(r=1.1,h=100,center=true,$fn=120);
		rotate([90, 0, 0]) translate([ 0, 0, 56])rotate([-50, 0, 0])translate([ -17, 17, -10])cylinder(r=1.1,h=100,center=true,$fn=120);
}

module scasso_ventola3() {
	rotate([90, 0, 120]) translate([ 0, 0, 56])rotate([-50, 0, 0])cube([40.2, 40.2, 8],center=true);
	rotate([90, 0, 120]) translate([ 0, 0, 56])rotate([-50, 0, 0])translate([ -20.1, -20.1, -4])cube([40.2, 40.2, 20]);
		//rotate([90, 0, 0]) translate([ 0, 0, 50])rotate([-50, 0, 0])translate([ -15, -15, -30])cube([30, 30, 20]);

	rotate([90, 0, 120]) translate([ 0, 0, 56])rotate([-50, 0, 0])translate([ 17, 17, -10])cylinder(r=1.1,h=100,center=true,$fn=120);
	rotate([90, 0, 120]) translate([ 0, 0, 56])rotate([-50, 0, 0])translate([ -17, 17, -10])cylinder(r=1.1,h=100,center=true,$fn=120);
}
