...
module platform() {
	...
	union() {
    	...
    	cylinder(r=5, h=3,  center=false, $fn=4*q); 		            	  // M10 fixation hole for Merlin hotend
			translate([0, 0, 3]) cylinder(r1=5, r2=9, h=10,  center=false, $fn=4*q); // M10 top 60 degree finish of hotend hole
			translate([0, 0, 13])cylinder(r=9, h=30,  center=false, $fn=4*q); //Filament path
		//	translate([0, 0, 12]) cylinder(r1=6, r2=8, h=2,  center=false, $fn=4*q); //Filament path entrance
		...
		difference() {  // Mount for Bowden connector
		translate([0, 0, 14]) cylinder(r=12, h=35,  center=false, $fn=4*q);
		translate([0, 0, 14]) cylinder(r=9, h=35,  center=false, $fn=4*q);
		} 
	}
}
